...
function setTab(tab){
  if(tab!=="tattica" && typeof exitPseudoFullscreen==="function" && pseudoFS) exitPseudoFullscreen();
...
// Fullscreen (con fallback per mobile Safari / browser senza Fullscreen API)
var pseudoFS = false; // var per permettere accesso prima dell'inizializzazione (setTab)
const FS = {
  request(el){
...
function enterPseudoFullscreen(){
  ...
  fsWrap.classList.add("pseudo-fullscreen");
...
function exitPseudoFullscreen(){
  ...
  fsWrap.classList.remove("pseudo-fullscreen");
  setTimeout(()=>{ try{ resizeInk(); clampTokens(); }catch(_){ } }, 80);
}

function requestBoardFullscreen(){
  // Prefer native Fullscreen API; if not available or denied, fallback
  const p = FS.request(fsWrap);

  // Alcuni browser mobile (anche Android in certe webview) possono:
  // - avere una funzione requestFullscreen "stub" che ritorna undefined
  // - non restituire una Promise
  // - fallire in modo silenzioso
  // Per questo verifichiamo dopo un breve delay se siamo davvero entrati
  // in fullscreen: se no, attiviamo il pseudo-fullscreen.
  if(p && typeof p.then === "function"){
    p.catch(()=> enterPseudoFullscreen());
    setTimeout(()=>{
      if(!FS.element() && !pseudoFS) enterPseudoFullscreen();
    }, 250);
    return;
  }
