...
// Presenze: giorno selezionato (per azioni rapide)
let attendanceSelectedIso = null;
function pickDefaultAttendanceDay(dates, monthKey){
  if(!dates || !dates.length) return null;
  const todayIso = new Date().toISOString().slice(0,10);
  const [y,m] = (monthKey||"").split("-");
  const inMonth = !!y && !!m && todayIso.startsWith(y + "-" + m);
  if(inMonth && dates.some(d=>d.iso===todayIso)) return todayIso;
  return dates[0].iso;
}
function renderAttendance(){
  ...
  if(!attendanceSelectedIso || !dates.some(d=>d.iso===attendanceSelectedIso)){
    attendanceSelectedIso = pickDefaultAttendanceDay(dates, monthKey);
  }
  ...
  for(const d of dates){
    ...
    if(attendanceSelectedIso && d.iso===attendanceSelectedIso) th.classList.add("sel");
    hr.appendChild(th);
  }
  ...
  for(const p of roster){
    ...
    for(const d of dates){
      ...
      if(attendanceSelectedIso && d.iso===attendanceSelectedIso) td.classList.add("sel");
...
function addToken(playerId){
  ...
  try{
    ...
    if(count>=7 && document.fullscreenElement==null && fsWrap.requestFullscreen){
      setTool("none");
      fsWrap.requestFullscreen().then(()=>{
        // quando cambia dimensione, ricalcola canvas e riposiziona i token
        setTimeout(()=>{ resizeInk(); clampTokens(); }, 150);
      }).catch(()=>{});
    }
  }catch(_){}
...
$("btnAllPresent").addEventListener("click", ()=>{
  ...
  const iso = attendanceSelectedIso;
  if(!iso) return;
...
$("btnClearDay").addEventListener("click", ()=>{
  ...
  const att = getAttendance(monthKey);
  for(const p of roster){
    if(att[p.id] && att[p.id][iso]) delete att[p.id][iso];
  }
  setAttendance(monthKey, att);
  renderAttendance();
});
