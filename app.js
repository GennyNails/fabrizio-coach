...
function setTab(tab){
  // Se esco da Tattiche, chiudi eventuale modalità presentazione/fullscreen.
  if(tab !== "tattica" && (isPresentMode() || document.fullscreenElement)){
    exitBoardFullscreen();
  }
...
async function requestNativeFullscreen(){
  try{
    if(!fsWrap?.requestFullscreen) return false;
    await fsWrap.requestFullscreen();
...
async function enterBoardFullscreen(){
  // garantisce sempre campo a schermo pieno (anche se il fullscreen nativo fallisce)
  setTool("none");
  enterPresentMode();
  // prova comunque il fullscreen nativo (Chrome/Android)
  const ok = await requestNativeFullscreen();
  if(ok){
    setTimeout(()=>{ try{ resizeInk(); clampTokens(); }catch(_){ } }, 120);
  }
}

async function exitBoardFullscreen(){
  // esce sia dal fullscreen nativo sia dal pseudo-fullscreen
  try{
