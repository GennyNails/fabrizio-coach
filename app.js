...
function setTab(tab){
  ...
  if(tab==="partite"){ renderMatchDropdowns(); renderWaDropdowns(); initWhatsAppConvocation(); renderMatches(); }
...
function renderRoster(){
  // aggiorna anche dropdown WhatsApp (se presenti)
...
function renderWaDropdowns(){
  const cap = $("wCaptain"), vice = $("wVice");
  if(!cap || !vice) return;

  // Se ho una lista convocati, mostro solo quelli in dropdown
  const last = Storage.get("whatsappLast", {}) || {};
  const allowed = Array.isArray(last.convocati) ? last.convocati : null;
  const players = (allowed && allowed.length)
    ? roster.filter(p=>allowed.includes(p.id))
    : roster;
...
function waRenderConvocatiList(selectedIds, captainId, viceId){
  ...
  for(const p of sorted){
    ...
    row.className = "wa-player";
    row.innerHTML = `
      <input type="checkbox" class="wa-cb" value="${Utils.escapeAttr(p.id)}" ${sel.has(p.id)?"checked":""} />
      <span class="wa-name">${Utils.escapeHtml(p.name||"—")}</span>
      <button class="wa-role" type="button" data-role="cap" data-id="${Utils.escapeAttr(p.id)}" aria-label="Imposta Capitano">C</button>
      <button class="wa-role" type="button" data-role="vice" data-id="${Utils.escapeAttr(p.id)}" aria-label="Imposta Vice">V</button>
    `;

    const capBtn = row.querySelector('[data-role="cap"]');
    const viceBtn = row.querySelector('[data-role="vice"]');
    if(p.id === captainId) capBtn.classList.add("active");
    if(p.id === viceId) viceBtn.classList.add("active");
...
function initWhatsAppConvocation(){
  ...
  renderWaDropdowns();
  if($("wCaptain")) $("wCaptain").value = last.captain || roster[0]?.id || "";
  if($("wVice")) $("wVice").value = last.vice || roster[0]?.id || "";

  // lista convocati
  const convocati = Array.isArray(last.convocati) ? last.convocati : [];
  waRenderConvocatiList(convocati, $("wCaptain")?.value || "", $("wVice")?.value || "");
  ...
  const recompute = ()=>{
    ...
    // aggiorna dropdown cap/vice in base ai convocati
    Storage.set("whatsappLast", {
      ...(Storage.get("whatsappLast", {})||{}),
      convocati: checked
    });
    renderWaDropdowns();

    // se cap/vice non sono nei convocati, forza al primo convocato (se presente)
    const capSel = $("wCaptain");
    const viceSel = $("wVice");
    if(checked.length){
      if(capSel && !checked.includes(capSel.value)) capSel.value = checked[0];
      if(viceSel && !checked.includes(viceSel.value)) viceSel.value = checked[0];
    }
    ...
    const values = [
      ...
      ($("wField")?.value || "").trim(),
      ($("wTime")?.value || "").trim(),
      ($("wCaptain")?.selectedOptions?.[0]?.textContent || "").trim(),
      ($("wVice")?.selectedOptions?.[0]?.textContent || "").trim(),
    ];
    const out = waFillTemplateSequential(tplEl.value, values);
    ...
    // aggiorna UI lista convocati (evidenzia C/V)
    waRenderConvocatiList(checked, $("wCaptain")?.value || "", $("wVice")?.value || "");
    return out;
  };
  ...
  ["input","change"].forEach(ev=>{
    ...
    $("wCaptain")?.addEventListener(ev, recompute);
    $("wVice")?.addEventListener(ev, recompute);
  ...
  $("waConvocati")?.addEventListener("click", (e)=>{
    ...
    const cb = $("waConvocati").querySelector(`.wa-cb[value="${CSS.escape(id)}"]`);
    if(cb) cb.checked = true;

    if(role === "cap" && $("wCaptain")) $("wCaptain").value = id;
    if(role === "vice" && $("wVice")) $("wVice").value = id;
    recompute();
  });
  ...
  $("btnWaFillNext")?.addEventListener("click", ()=>{
    ...
    if(nm){
      ...
      if(nm.captain) $("wCaptain").value = nm.captain;
      if(nm.vice) $("wVice").value = nm.vice;
