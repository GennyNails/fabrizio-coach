...
function attachTokenDrag(tokenEl){
  ...
  const onMove = (e) => {
    ...
    const x = (p.x - r.left) - tw/2;
    const y = (p.y - r.top)  - th/2;
  ...
  window.addEventListener("touchmove", (e)=>{
    ...
    // keep only the active finger
    const list = e.touches;
    if(!list) return;
    let found = false;
    for(let i=0;i<list.length;i++) if(list[i].identifier===activeTouchId){ found=true; break; }
    if(!found) return;
    onMove(e);
  }, {passive:false});
  window.addEventListener("touchend", ()=>{ draggingLocal=false; activeTouchId=null; }, {passive:false});
  window.addEventListener("touchcancel", ()=>{ draggingLocal=false; activeTouchId=null; }, {passive:false});
}
...
// draw/erase (mobile-safe: pointer + touch fallback)
...
function startInk(e){
  ...
  // important for iOS: prevent scroll/zoom while drawing
  if(e.cancelable) e.preventDefault();
}
...
// Touch fallback (old iOS / embedded webviews)
ink.addEventListener("touchstart", startInk, {passive:false});
ink.addEventListener("touchmove", moveInk, {passive:false});
ink.addEventListener("touchend", endInk, {passive:false});
ink.addEventListener("touchcancel", endInk, {passive:false});
