...
function setTab(tab){
  ...
  if(tab==="tattica"){ renderBoardRoster(); resizeInk(); setTimeout(()=>clampTokensPx(), 60); }
...
function clampTokensPx(){
  ...
  tokens.querySelectorAll(".token").forEach(t=>{
    ...
    x = Math.max(0, Math.min(x, pw - tw));
    y = Math.max(0, Math.min(y, ph - th));
    t.style.left = x + "px";
    t.style.top  = y + "px";
  });
}
...
window.addEventListener("resize", ()=>{
  if(!pages.tattica.classList.contains("hidden")){
    resizeInk();
    setTimeout(()=>clampTokensPx(), 60);
  }
});
document.addEventListener("fullscreenchange", ()=>{
  if(!pages.tattica.classList.contains("hidden")){
    resizeInk();
    setTimeout(()=>clampTokensPx(), 120);
  }
});
...
function addToken(playerId){
  ...
  // posizione iniziale al centro, dentro al campo
  const pw = pitch.clientWidth;
  const ph = pitch.clientHeight;
  const tw = tokenEl.offsetWidth || 48;
  const th = tokenEl.offsetHeight || 48;
  tokenEl.style.left = Math.max(0, pw*0.5 - tw*0.5) + "px";
  tokenEl.style.top  = Math.max(0, ph*0.5 - th*0.5) + "px";
  clampTokensPx();
  ...
  try{
    ...
    if(count>=7 && document.fullscreenElement==null && fsWrap.requestFullscreen){
      ...
      fsWrap.requestFullscreen().then(()=>{
        setTimeout(()=>{ resizeInk(); clampTokensPx(); }, 120);
      }).catch(()=>{});
    }
  }catch(_){}
  ...
  tokenEl.addEventListener("pointermove",(e)=>{
    ...
    nx = Math.max(0, Math.min(nx, pw - tw));
    ny = Math.max(0, Math.min(ny, ph - th));
  ...
  tokenEl.addEventListener("pointercancel", ()=> dragging=false);
}
