...
// NB: versione richiesta: "Gestione Squadra 3.0"
const VERSION = "3.1";
...
const Storage = {
  ...
  // Protezione quota/permessi: evita crash se lo storage è pieno o bloccato.
  set(key, value){
    try{
      localStorage.setItem(key, JSON.stringify(value));
      return true;
    }catch(err){
      // QuotaExceededError (Safari/iOS può essere aggressivo, specie con foto in dataURL)
      try{
        const msg = "Memoria locale piena o non disponibile.\n\nSuggerimento: riduci/elimina foto personalizzate o libera spazio.";
        alert(msg);
      }catch(_){ /* no-op */ }
      return false;
    }
...
// Escape HTML per evitare injection quando usiamo template string in innerHTML
...
thead.appendChild(hr);
  ...
  for(const p of roster){
    ...
    if(_parts.length>1){
      ...
      nm.innerHTML = `<span class="fn">${escapeHtml(_first)}</span><span class="ln">${escapeHtml(_last)}</span>`;
    }else{
      nm.textContent = _n;
    }
    cell.appendChild(nm);
...
function renderRoster(){
  ...
  for(const p of roster){
    ...
    nm.innerHTML = `<span class="first">${escapeHtml(first)}</span><span class="last">${escapeHtml(last)}</span>`;
...
function renderMatches(){
  ...
  for(const m of sorted){
    ...
    const oppSafe = escapeHtml(m.opponent || "Avversario");
    const capSafe = escapeHtml(capName);
    const viceSafe = escapeHtml(viceName);
    const dateSafe = escapeHtml(date);
    const scoreSafe = escapeHtml(score);
    el.innerHTML = `
      <div>
        <div class="t">${oppSafe}</div>
        <div class="s">${dateSafe} • ${scoreSafe}</div>
        <div class="s">Capitano: ${capSafe} • Vice: ${viceSafe}</div>
      </div>
      <div class="a">
...
function buildMatchesPrintTable(){
  ...
  for(const m of sorted){
    ...
    const tdO=document.createElement("td"); tdO.textContent = (m.opponent||"");
    const tdS=document.createElement("td"); tdS.textContent = score;
    const tdC=document.createElement("td"); tdC.textContent = capName;
    const tdV=document.createElement("td"); tdV.textContent = viceName;
    tr.appendChild(tdD);
    tr.appendChild(tdO);
    tr.appendChild(tdS);
    tr.appendChild(tdC);
...
// Condivisione sola lettura (Presenze + Rosa)
...
function buildReadOnlySharePayload(){
  // Roster: include anche le immagini (asset e foto custom) per vista identica.
  ...
  // Presenze: prende TUTTI i mesi presenti nello storage (att:YYYY-MM)
  const attendance = {};
  try{
    for(let i=0;i<localStorage.length;i++){
      const k = localStorage.key(i);
      if(!k) continue;
      if(k.startsWith("att:")){
        attendance[k.slice(4)] = Storage.get(k, {});
      }
    }
  }catch(_){ /* no-op */ }

  return {
    app: "Gestione Squadra",
    version: VERSION,
    generatedAt: new Date().toISOString(),
    team: {
      teamName: settings.teamName || "Gestione Squadra",
      category: settings.category || "",
      misterName: settings.misterName || ""
    },
    roster: rosterView,
    attendance
  };
...
async function shareReadOnlyLink(){
  try{
    ...
    const base = location.href.split('#')[0].replace(/index\.html(\?.*)?$/i, "");
    const url = base + "share.html#" + encoded;

    // prova Web Share API, altrimenti copia negli appunti
    if(navigator.share){
      await navigator.share({
        title: "Gestione Squadra — sola lettura",
        text: "Presenze + Rosa (solo visualizzazione)",
        url
      });
      alert("Link condiviso.");
      return;
    }
    if(navigator.clipboard?.writeText){
      await navigator.clipboard.writeText(url);
      alert("Link copiato negli appunti.\n\nIncollalo su WhatsApp/Email per condividerlo.");
      return;
    }
    // fallback ultimo
    prompt("Copia questo link:", url);
  }catch(err){
    alert("Impossibile creare il link di condivisione: " + String(err?.message||err));
  }
}

const btnShare = document.getElementById("btnShareReadOnly");
if(btnShare){ btnShare.addEventListener("click", shareReadOnlyLink); }
