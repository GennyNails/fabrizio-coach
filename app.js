...
if(btnToggleRoster){
  btnToggleRoster.addEventListener("click", ()=>{
    ...
    if(!o) return;
    setRosterCollapsed(!o.classList.contains("roster-collapsed"));
  });
}
...
function renderAttendance(){
  ...
  // Layout richiesto: pallini "in verticale".
  // Mostriamo una riga per ogni giorno di allenamento (verticale), con una colonna per ogni giocatore.
  const wrap = $("attendanceTable");
  wrap.innerHTML = "";
  const table = document.createElement("table");
  table.className = "att-vertical";

  const thead = document.createElement("thead");
  const hr = document.createElement("tr");
  const th0 = document.createElement("th");
  th0.textContent = "Giorno";
  hr.appendChild(th0);
  ...
  for(const d of dates){
    ...
    tr.dataset.date = d.iso;
    if(d.iso===todayIso) tr.classList.add("today");
    if(d.iso===selectedIso) tr.classList.add("selected");

    const thDay = document.createElement("th");
    thDay.className = "day-head";
    thDay.dataset.date = d.iso;
    thDay.textContent = `${Utils.itWdShort(d.wd)} ${d.day}`;
    if(d.iso===todayIso) thDay.classList.add("today");
    if(d.iso===selectedIso) thDay.classList.add("selected");
...
function openBoardOverlay(){
  const o = document.getElementById("boardOverlay");
  if(!o) return;
  // su mobile apri con rosa estesa; verrà poi compressa automaticamente dopo 7 giocatori
...
async function enterBoardFullscreen(){
  // prova il fullscreen nativo (PC + Android Chrome). Se non disponibile, l'overlay è già a schermo intero.
  setTool("none");
  const ok = await requestNativeFullscreen();
  if(ok) setTimeout(()=>{ try{ resizeInk(); clampTokens(); }catch(_){ } }, 120);
}
...
function buildAttendancePrintTable(){
  ...
  for(const d of dates){
    const th = document.createElement("th");
    th.textContent = d.label; // es. Lun 2
    trh.appendChild(th);
  }
  ...
  for(const p of roster){
    ...
    tdN.className = "sticky";
    row.appendChild(tdN);

    let count = 0;
    for(const d of dates){
      const td = document.createElement("td");
      const on = !!(att[p.id] && att[p.id][d.key]);
      if(on) count++;
      td.textContent = on ? "●" : "";
      td.className = on ? "on" : "";
      row.appendChild(td);
...
(function(){
  try {
    ...
    if(!localStorage.getItem('matches')){
      localStorage.setItem('matches', JSON.stringify([{"date": "2026-02-11", "opponent": "Orsago", "home": true, "played": true, "periods": [[3, 0], [2, 0], [2, 0], [2, 2]]}, {"date": "2026-02-18", "opponent": "Uniongaia", "home": true, "played": false}, {"date": "2026-02-25", "opponent": "Vittsangiacomo", "home": false, "played": false}, {"date": "2026-03-04", "opponent": "Cappella Magg.", "home": true, "played": false}, {"date": "2026-03-11", "opponent": "Dinamis", "home": true, "played": false}, {"date": "2026-03-18", "opponent": "San Fior", "home": false, "played": false}, {"date": "2026-03-25", "opponent": "Godega", "home": true, "played": false}, {"date": "2026-04-08", "opponent": "Orsago", "home": false, "played": false}, {"date": "2026-04-15", "opponent": "Uniongaia", "home": false, "played": false}]));
    }
  } catch(e){}
})();
