...
const STORE_KEY = "mister_soccer_v19";
...
function renderAttendanceMatrix(){
  ...
  if(!el.attendanceMatrixWrap) return;
  ...
  for(const d of dates){
    ...
    th.innerHTML = `<span class="daypill">${day}</span><span class="wk">${escapeHtml(wk)}</span>`;
    hr.appendChild(th);
  }
  ...
  for(const name of rosterNames()){
    ...
    for(const d of dates){
      ...
      chk.type = "checkbox";
      chk.checked = !!(state.attendance[d] && state.attendance[d][name]);
      chk.dataset.date = d;
      chk.dataset.name = name;

      // pointer/touch friendly
      chk.addEventListener("change", ()=>{
        setPresent(d, name, chk.checked);
        renderPercentages(); // live update
        renderMonthlyReport(); // live update if open
      });
  ...
  el.attendanceMatrixWrap.innerHTML = "";
  el.attendanceMatrixWrap.appendChild(table);
}

function saveAttendanceMonth(){
  ensureAttendanceShape();
  // niente trasformazioni: già salvato in memoria mentre spunti. Qui forziamo solo persistenza + reset dirty.
  saveState();
...
function setMonthInputDefaults(){
  ...
  if(el.monthPick && !el.monthPick.value) el.monthPick.value = m;
  if(el.monthlyReportMonth && !el.monthlyReportMonth.value) el.monthlyReportMonth.value = m;
}
...
function renderMonthlyReport(){
  ...
  const month = el.monthlyReportMonth?.value || monthISO(todayISO());
  const dates = monthTrainingDates(month);
  const total = dates.length;
...
if(el.monthlyReportMonth){
  el.monthlyReportMonth.addEventListener("change", ()=>{ renderMonthlyReport(); });
}
