...
// Versione applicazione (mostrata in Impostazioni e intestazione stampa)
const VERSION = "1.2.0";
...
// Per la lavagna: prova a rimuovere automaticamente lo sfondo blu dalle foto (solo in tattiche)
function avatarNodeBoard(player){
  // base avatar (foto o fallback)
  const node = avatarNode(player);
  // Se non è una foto caricata, non fare nulla
  if(!player.photoDataUrl) return node;

  // Cache locale per evitare ricalcoli (vale per lavagna)
  try{
    const h = Utils.hashString(player.photoDataUrl);
    const cacheKey = "cutout:"+player.id+":"+h;
    const cached = Storage.get(cacheKey, "");
    if(cached){
      if(node && node.tagName==="IMG") node.src = cached;
      return node;
    }

    // calcola in async e salva in cache
    Utils.blueToTransparent(player.photoDataUrl).then((cut)=>{
      try{
        Storage.set(cacheKey, cut);
      }catch(_){}
      if(node && node.tagName==="IMG") node.src = cut;
    });
  }catch(_){}
  return node;
}
...
// (La lavagna ora è "pura": nessuna rosa laterale)
...
function openBoardOverlay(){
  ...
  setTool("none");
  // crea le pedine base (6 rosse + 1 gialla) la prima volta
  initBoardTokensOnce();
  setTimeout(()=>{ try{ updateViewportVars(); resizeInk(); clampTokens(); }catch(_){ } }, 60);
}
...
async function enterBoardFullscreen(){
  // La lavagna è già fullscreen tramite overlay + CSS (più stabile su mobile)
  setTool("none");
  setTimeout(()=>{ updateViewportVars(); safeBoardResize(); }, 80);
  setTimeout(()=>{ updateViewportVars(); safeBoardResize(); }, 240);
}
...
function resizeInk(){
  ...
  // +20% rispetto al tratto precedente
  ctx.lineWidth = 3.6;
  ctx.lineCap = "round";
  ctx.strokeStyle = "rgba(255,45,45,.95)"; // penna rossa
...
document.addEventListener("fullscreenchange", ()=>{
  if(isBoardOpen()){
    resizeInk();
    setTimeout(()=>clampTokens(), 120);
  }
});

function renderBoardRoster(){
  const wrap = $("boardRoster");
  if(!wrap) return; // la lavagna ora non usa la rosa
  wrap.innerHTML = "";
  for(const p of roster){
...
function setRosterCollapsed(collapsed){
  const o = document.getElementById("boardOverlay");
  if(!o) return;
  o.classList.toggle("roster-collapsed", !!collapsed);
  // ricalcola subito canvas/token (il layout cambia)
  setTimeout(()=>{ try{ resizeInk(); clampTokens(); }catch(_){ } }, 60);
}
...
function safeBoardResize(){
  try{ resizeInk(); clampTokens(); }catch(_){}
}

// --- Lavagna "pura": pedine predefinite (6 rosse + 1 gialla) ---
let boardTokensInitialized = false;

function createJerseyToken(imgUrl, nx, ny){
  const tokenEl = document.createElement("div");
  tokenEl.className = "token";
  tokenEl.style.setProperty("--token-img", `url('${imgUrl}')`);
  ...
  attachTokenDragHandlers(tokenEl);
  return tokenEl;
}

function initBoardTokensOnce(){
  if(boardTokensInitialized) return;
  boardTokensInitialized = true;
  tokens.innerHTML = "";

  const red = "assets/jersey_red.png";
  const yellow = "assets/jersey_yellow.png";

  const startX = 0.06;
  const ys = [0.12, 0.24, 0.36, 0.48, 0.60, 0.72];
  ys.forEach(y=>createJerseyToken(red, startX, y));
  createJerseyToken(yellow, 0.22, 0.86);
}

function attachTokenDragHandlers(tokenEl){
  let dragging = false;
  let startX = 0, startY = 0;
  let origNX = 0.5, origNY = 0.5;
  ...
  function onMove(e){
    ...
    const pw = pitch.clientWidth;
    const ph = pitch.clientHeight;
    const tw = tokenEl.offsetWidth || 56;
    const th = tokenEl.offsetHeight || 56;
    const aw = Math.max(1, pw - tw);
    const ah = Math.max(1, ph - th);
  ...
  function startDrag(e){
    ...
    const {x,y} = getXY(e);
    startX = x; startY = y;
    origNX = parseFloat(tokenEl.dataset.nx || "0.5");
    origNY = parseFloat(tokenEl.dataset.ny || "0.5");
  ...
  tokenEl.addEventListener("pointerdown", startDrag, {passive:false});
  tokenEl.addEventListener("mousedown", startDrag, {passive:false});
  tokenEl.addEventListener("touchstart", startDrag, {passive:false});
}


function updateBoardTokenNumbers(playerId){
  try{
    const p = roster.find(x=>x.id===playerId);
    const num = (p && p.number && String(p.number).trim()!=="") ? String(p.number) : "";
    const nodes = document.querySelectorAll('.token[data-playerid="'+playerId+'"] .token-num');
    nodes.forEach(n=>{
      n.textContent = num;
...
function addToken(playerId){
  ...
  const tokenEl=document.createElement("div");
  tokenEl.className="token";
  tokenEl.dataset.playerid = String(playerId);
  tokenEl.appendChild(avatarNodeBoard(p));

  const numEl=document.createElement("div");
  numEl.className="token-num" + ((p.number && String(p.number).trim()!=="") ? "" : " hidden");
  numEl.textContent = (p.number && String(p.number).trim()!=="") ? String(p.number) : "";
  tokenEl.appendChild(numEl);

  tokens.appendChild(tokenEl);

  // Posizione iniziale: centro campo. Per evitare che alcuni "spariscano" perché sovrapposti,
  // li disponiamo in un piccolo cluster attorno al centro (sempre al centro-area), poi puoi spostarli.
  requestAnimationFrame(()=>{
    const count = tokens.querySelectorAll(".token").length; // include questo
    const k = Math.max(0, count - 1);
    const col = (k % 3) - 1;              // -1,0,1
    const row = (Math.floor(k/3) % 3) - 1; // -1,0,1
    const dx = col * 0.045;
    const dy = row * 0.045;
    const nx = Math.max(0.05, Math.min(0.95, 0.5 + dx));
    const ny = Math.max(0.05, Math.min(0.95, 0.5 + dy));
    setTokenPos(tokenEl, nx, ny);
    clampTokens();
  });

  // Da 7 giocatori in poi entra in modalità presentazione (campo a schermo pieno).
  // Su mobile, se il fullscreen nativo fallisce, la present-mode garantisce comunque lo schermo pieno.
  try{
    const count = tokens.querySelectorAll(".token").length;
    if(count>=7 && !isPresentMode()){
      enterBoardFullscreen();
    }
    // su mobile, dopo 7 giocatori comprimi la lista rosa per massimizzare il campo
    if(count>=7 && isMobileBoard()){
      setRosterCollapsed(true);
    }
  }catch(_){ }

  // Drag robusto (desktop + mobile): usa listener su window per evitare problemi in fullscreen
  let startX=0,startY=0,origNX=0.5,origNY=0.5,dragging=false,activePid=null;

  function onMove(e){
    if(!dragging) return;
    if(activePid!=null && e.pointerId!=null && e.pointerId!==activePid) return;
...
// draw/erase (desktop + mobile touch)
function boardPointFromEvent(e){
  const r = ink.getBoundingClientRect();
  let x = e.clientX, y = e.clientY;
  if(e.touches && e.touches[0]){ x = e.touches[0].clientX; y = e.touches[0].clientY; }
  if(e.changedTouches && e.changedTouches[0]){ x = e.changedTouches[0].clientX; y = e.changedTouches[0].clientY; }
...
function boardDrawStart(e){
  ...
  if(e.cancelable) e.preventDefault();
  drawing = true;
  if(e.pointerId!=null && ink.setPointerCapture) {
...
function boardDrawEnd(){ drawing = false; }

ink.addEventListener("pointerdown", boardDrawStart, {passive:false});
ink.addEventListener("pointermove", boardDrawMove, {passive:false});
ink.addEventListener("pointerup", boardDrawEnd, {passive:true});
ink.addEventListener("pointercancel", boardDrawEnd, {passive:true});

ink.addEventListener("touchstart", boardDrawStart, {passive:false});
ink.addEventListener("touchmove", boardDrawMove, {passive:false});
ink.addEventListener("touchend", boardDrawEnd, {passive:true});
ink.addEventListener("touchcancel", boardDrawEnd, {passive:true});

ink.addEventListener("mousedown", boardDrawStart, {passive:false});
window.addEventListener("mousemove", (e)=>{ if(drawing) boardDrawMove(e); }, {passive:false});
window.addEventListener("mouseup", boardDrawEnd, {passive:true});
...
function buildAttendancePrintTable(){
  ...
  // Stampa presenze: SOLO tabella dati
  const table = document.createElement("table");
...
function buildRosterPrintTable(){
  ...
  table.innerHTML = "<thead><tr><th>Giocatore</th><th>Numero</th><th>Ruolo</th></tr></thead>";
  const tbody = document.createElement("tbody");
  for(const p of roster){
    const tr = document.createElement("tr");
    const num = (p.number && String(p.number).trim()!=="") ? String(p.number) : "—";
    const role = (p.role && String(p.role).trim()!=="") ? String(p.role) : "—";
    tr.innerHTML = `<td>${p.name}</td><td>${num}</td><td>${role}</td>`;
    tbody.appendChild(tr);
  }
  table.appendChild(tbody);
  printArea.appendChild(table);
...
function preparePrint(which){
  ...
  if(which==="presenze"){
    ...
    // per ora, solo le stampe richieste: presenze e partite
    setPrintHeader("");
  }
}
