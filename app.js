...
const STORE_KEY = "mister_soccer_v18";
...
function ensureMonthGenerated() {
  // Nessuna generazione automatica: le giornate si aggiungono manualmente.
...
function addExtraDay() {
  const month = getSelectedMonth();
  let date = el.extraDate.value;
  if(!date){
    date = (prompt("Inserisci data extra (YYYY-MM-DD):","")||"").trim();
    if(!date) return;
  }
  ...
  const newId = uid();
  md.sessions.push({ id: newId, date, marks: {} });
  selectedSessionId = newId;
  if(el.extraDate) el.extraDate.value = "";
...
function renderSessions(){
  const month = getSelectedMonth();
  const md = getMonthData(month);
  const sessions = (md.sessions || []).slice().sort((a,b)=>(a.date||"").localeCompare(b.date||""));
  ...
  if(el.sessionsTbody){
    ...
    if(!sessions.length){
      el.sessionsTbody.innerHTML = `<tr><td colspan="3" class="muted center">Nessuna giornata nel mese. Usa “Aggiungi extra”.</td></tr>`;
    } else {
      for(const s of sessions){
        const mk = s.marks || {};
        const present = Object.values(mk).filter(v=>v==="P").length;
        const total = rosterNames().length;
        const tr = document.createElement("tr");
        tr.innerHTML = `
          <td>${escapeHtml(formatDateLong(s.date))}</td>
          <td class="right"><span class="badge">${present}/${total}</span></td>
          <td class="right">
            <button class="btn small" data-edit="${s.id}">Modifica</button>
            <button class="btn small danger" data-del="${s.id}">X</button>
          </td>
        `;
        tr.querySelector('[data-edit]').addEventListener("click", ()=>{
          // se cambi sessione con modifiche non salvate, le manteniamo in memoria ma avvisiamo
          selectedSessionId = s.id;
          renderSessions();
          renderAttendanceEditor();
          // scroll verso editor
          document.getElementById("selectedSessionBadge")?.scrollIntoView({behavior:"smooth", block:"start"});
        });
        tr.querySelector('[data-del]').addEventListener("click", ()=>{
          if(confirm("Eliminare questa giornata?")) removeSession(month, s.id);
        });
  ...
  el.selectedSessionBadge.textContent = sel ? formatDateLong(sel.date) : "—";
...
function renderAttendanceEditor(){
  ...
  for(const name of rosterNames()){
    ...
    chk.type = "checkbox";
    chk.checked = (session.marks?.[name] === "P");
    chk.addEventListener("change", ()=>{
      setMark(month, sid, name, chk.checked ? "P" : "");
      // aggiorna subito
      renderPercentages();
      renderReport(); 
    });
...
// ----------------- EVENTS -----------------
// el.ensureMonthBtn: generazione automatica disattivata
...
if(el.saveAttendanceBtn){
  el.saveAttendanceBtn.addEventListener("click", ()=>{ saveState(); attendanceDirty=false; updateSaveAttendanceUI(); renderReport(); toast("Salvato ✓"); });
}
