...
function renderBoardRoster(){
  ...
  for(const p of roster){
    ...
    av.textContent = Utils.initials(p.name);
    ...
    nr.className="chip-num" + (num?"":" hidden");
    nr.textContent = num;
    chip.appendChild(nr);
...
function addToken(playerId){
  ...
  try{
    ...
    // dopo 7 giocatori comprimi la lista rosa per massimizzare il campo (tutti i dispositivi)
    if(count>=7){
