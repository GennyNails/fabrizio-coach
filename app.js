/* Gestione Squadra - v1.24 */
const VERSION = "1.25";

// Fullscreen helpers (con fallback "pseudo-fullscreen" per mobile)
// Nota: su iOS Safari il fullscreen vero spesso non è disponibile.
var pseudoFS = false;
const FS = {
  request(el){
...
function enterPseudoFullscreen(){
  ...
  if(fsWrap) fsWrap.classList.add("pseudo-fullscreen");
  // dopo il reflow, aggiorna canvas e limiti
  setTimeout(()=>{ try{ resizeInk(); clampTokens(); }catch(_){ } }, 80);
}
...
function exitPseudoFullscreen(){
  ...
  const fsWrap = document.getElementById("fsWrap");
  if(fsWrap) fsWrap.classList.remove("pseudo-fullscreen");
  setTimeout(()=>{ try{ resizeInk(); clampTokens(); }catch(_){ } }, 80);
}
...
function requestBoardFullscreen(){
  const fsWrap = document.getElementById("fsWrap");
  if(!fsWrap) return;
  // Prova fullscreen nativo (Chrome/Android lo supporta) e, se fallisce,
  // usa un pseudo-fullscreen affidabile (overlay fixed).
  let attempted = false;
  try{
    const p = FS.request(fsWrap);
    attempted = true;
    // Alcuni browser non ritornano una Promise: gestiamo comunque il fallback sotto.
    if(p && typeof p.catch === "function"){
      p.catch(()=>{ /* fallback sotto */ });
    }
  }catch(_){ /* fallback sotto */ }

  // Se dopo un attimo non siamo davvero in fullscreen, entra nel pseudo-fullscreen.
  setTimeout(()=>{
    if(!FS.element() && !pseudoFS){
...
// Protezione: se per qualche motivo la rosa salvata è vuota/corrotta,
// ripristina la rosa di default (altrimenti Presenze risulta "vuota").
let roster = Storage.get("roster", null);
if(!Array.isArray(roster) || roster.length===0){
  roster = DEFAULT_ROSTER.slice();
  try{ Storage.set("roster", roster); }catch(_){ }
}
...
function setTab(tab){
  // Se uscivamo dalla tattica in pseudo-fullscreen (mobile), ripristina la UI
  if(tab!=="tattica"){ try{ exitPseudoFullscreen(); }catch(_){ } }
  ...
  if(tab==="tattica"){ renderBoardRoster(); resizeInk(); setTimeout(()=>clampTokens(), 60); }
...
function renderAttendance(){
  ...
  // su mobile assicurati di partire dalla colonna giocatori
...
// Selezione giorno cliccando sull'intestazione
$("attendanceTable").addEventListener("click", (e)=>{
  const th = e.target.closest("th");
  if(!th) return;
  const iso = th.dataset?.date;
  if(!iso) return;
  const monthKey = $("monthSelect").value;
...
function resizeInk(){
  ...
  ctx.lineWidth = 3;
  ctx.lineCap = "round";
  ctx.strokeStyle = "rgba(40,120,255,.95)"; // penna blu
  ink.style.pointerEvents = (tool==="pen" || tool==="eraser") ? "auto" : "none";
}
window.addEventListener("resize", ()=>{
  if(!pages.tattica.classList.contains("hidden")){
    resizeInk();
...
function addToken(playerId){
  ...
  // posizione iniziale al centro (normalizzata) per restare corretta anche in fullscreen/resize
  // (aspetta un tick per misurare dimensioni)
  requestAnimationFrame(()=>{
    setTokenPos(tokenEl, 0.5, 0.5);
    clampTokens();
  });

  // Fullscreen automatico dal 7° giocatore (pitch + pulsanti). Tool OFF per permettere drag.
  try{
    const count = tokens.querySelectorAll(".token").length;
    if(count>=7 && !FS.element() && !pseudoFS){
      setTool("none");
      requestBoardFullscreen();
      // quando cambia dimensione (native o pseudo), ricalcola canvas e riposiziona i token
      setTimeout(()=>{ try{ resizeInk(); clampTokens(); }catch(_){ } }, 150);
    }
  }catch(_){ }
...
// Boot
// Allinea la versione salvata per mostrare correttamente il numero nell'header
...
// Sicurezza: su mobile, se l'app va in background mentre siamo in pseudo-fullscreen,
// alla ripresa ripristina la UI per evitare classi "appiccicate".
document.addEventListener("visibilitychange", ()=>{
  if(document.hidden && pseudoFS) exitPseudoFullscreen();
});
window.addEventListener("pagehide", ()=>{ if(pseudoFS) exitPseudoFullscreen(); });

// Exit fullscreen button
const btnExitFS = document.getElementById("btnExitFS");
if(btnExitFS){
  btnExitFS.addEventListener("click", ()=>{
    // Uscita sempre affidabile: se siamo in pseudo-fullscreen esci da quello,
    // altrimenti prova a chiudere il fullscreen nativo.
    if(pseudoFS){
    ...
    const el = FS.element();
    if(el){
      try{ const r = FS.exit(); if(r && r.catch) r.catch(()=>{}); }catch(_){ }
    }
  });
}
